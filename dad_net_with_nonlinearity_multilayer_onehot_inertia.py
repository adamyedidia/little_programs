...
#	print(X.shape)
	...
	return prevLamda + (inertiaFraction)*alpha*(prevY - np.dot(prevW, X))

def muStep(prevMu, prevZ, fOfPrevY):
	return prevMu + (1-inertiaFraction)*alpha*(prevZ - fOfPrevY)
...
#	return T - mu - rho*(prevZ - fOfPrevY)
	...
	return inertiaFraction*prevZ + (1-inertiaFraction)*(-mu/rho + fOfPrevY + T)/2
...
#		rhalpha*np.dot(np.transpose(WNextLayer), YNextLayer - np.dot(WNextLayer, prevZThisLayer)))
	...
	return inertiaFraction*np.transpose(diminishWithBias(np.transpose(prevZThisLayer))) + (1-inertiaFraction)* \
	(-muThisLayer/rho + fOfPrevYThisLayer + np.dot(np.transpose(diminishWithBias(WNextLayer)), lamdaNextLayer)/rho + \
		np.dot(np.transpose(diminishWithBias(WNextLayer)), YNextLayer - np.dot(WNextLayer, prevZThisLayer)))
...
def YStep(prevW, X, mu, fPrimeOfPrevY, lamda, Z, fOfPrevY, prevY):
	if False:
		print("")
		print("Y exploration")
	...
	return inertiaFraction*prevY + \
		(1-inertiaFraction)*(np.dot(prevW, X) + mu*fPrimeOfPrevY/rho - lamda/rho + fPrimeOfPrevY*(Z - fOfPrevY))
...
#		print("Z",layer, Zs[layer])
		...
		if layer == 0:
			Ys[layer] = YStep(prevWs[layer], biasAugmentedX, mus[layer], fPrimeOfPrevYs[layer], \
				lamdas[layer], Zs[layer], fOfPrevYs[layer], prevYs[layer])
		else:
